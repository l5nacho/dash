...
def compute_data_choice_1(df):
    ...
    tree_data = df.groupby(['DestState', 'Reporting_Airline'])['Flights'].sum().fillna(0).reset_index()
...
def compute_data_choice_2(df):
    ...
    avg_car = df.groupby(['Month', 'Reporting_Airline'])['CarrierDelay'].mean().fillna(0).reset_index()
    avg_weather = df.groupby(['Month', 'Reporting_Airline'])['WeatherDelay'].mean().fillna(0).reset_index()
    avg_NAS = df.groupby(['Month', 'Reporting_Airline'])['NASDelay'].mean().fillna(0).reset_index()
    avg_sec = df.groupby(['Month', 'Reporting_Airline'])['SecurityDelay'].mean().fillna(0).reset_index()
    avg_late = df.groupby(['Month', 'Reporting_Airline'])['LateAircraftDelay'].mean().fillna(0).reset_index()
...
app.layout = html.Div(children=[
    ...
    html.H1('US Domestic Airline Flights Performance',
            style={'textAlign': 'center', 'color': '#503D36', 'fomt-size': 24}),
